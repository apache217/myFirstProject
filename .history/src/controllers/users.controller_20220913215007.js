...
class UserController {
  ...
  async createUser(req, res) {
    if (req.body.user && req.body.user.id) {
      ...
      if (result) return res.status(200).send({user: req.users[req.body.user.id]});
  ...
  async updateUser(req, res) {
    if (req.body.user && req.body.user.id || req.body.user && req.body.user.id === 0) {
      if (!req.users.hasOwnProperty(req.body.user.id))
        return res.status(404).send({ message: "User not found." });
  ...
  async deleteUser(req, res) {
    ...
    if (req.params.id || req.params.id) {
      if (req.users.hasOwnProperty(req.params.id)) {
        let index = req.users.findIndex(array, predicate, fromIndex)()
